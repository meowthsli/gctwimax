		...
		for (i = 0x04;i < len; )
		{
			...
			switch (buf[i]){
				...
				case 0x1d: {	//	-- Device SW Version :xxxx
							for (j = i+2; j < param_len + i + 2; j++)
							{
								dev->firmware[j - (i + 2) + 5] = buf[j] + 0x30;
							}
		        ...
		        case 0xd2:	//	-- NSP ID
					if (param_len == 0x3) {
						memcpy(dev->nspid, buf + i + 2, param_len);
...
*/						
				default: {
						wmlog_msg(1, "bad config format %02x",buf[i]);
						break;
						//return -1;
					}
			}
			if (buf[i] != 0xd1) i += buf[i+1] + 2;
...
int fill_get_cert_req(unsigned char *buf, struct wimax_dev_status *dev)
{
	...
	if(dev->cert[1] == 0xff)
	{
		param[0] = 0x01;
		param[1] = 0x01;
		return fill_normal_config_req(buf, 0x03, 0x12, 6, param);
	}
	if(dev->cert[2] == 0xff && dev->cert[1] < 0x06)
	{
		param[0] = dev->cert[0];
		param[1] = (dev->cert[1]+1 == 5) ? 0x06 : dev->cert[1]+1;
		return fill_normal_config_req(buf, 0x03, 0x12, 6, param);
	}
	if(dev->cert[2] == 0x00)
	{	
		memcpy(param,dev->cert,6);
		return fill_normal_config_req(buf, 0x03, 0x14, sizeof(param), param);
	}
	return 0;
}
/* delete
int fill_homenet_req(unsigned char *buf) // * Ok
{
